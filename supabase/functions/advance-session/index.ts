...
// Only the judge or host can advance sessions or jump to verdict.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { roomId, target = 'next', expectedSession } = body
    ...
    if (!isJudge && !isHost) throw new Error('Only the judge or host can advance sessions')
    ...
    if (typeof expectedSession === 'number' && room.current_session !== expectedSession) {
      return new Response(
        JSON.stringify({ ok: true, idempotent: true, stale_request: true, status: room.status, current_session: room.current_session }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      )
    }

    const jumpToVerdict = target === 'verdict' || room.current_session >= 3
    let update: Record<string, unknown>
    let systemMsg: string
    let next: string | number

    if (jumpToVerdict) {
      update = { status: 'verdict', session_ends_at: null }
      systemMsg = 'انتهت الجلسات — انتقل الجميع إلى مرحلة الحكم الآن'
      next = 'verdict'
    } else {
      ...
      systemMsg = `بدأت ${nextSession === 2 ? 'جلسة المرافعات' : `الجلسة ${nextSession}`}`
      next = nextSession
    }
